...
carriage_dim_add=[-4,0,2+carriage_filament_corr];
...
carriage_tr=[[mk8_body_tr[0].x-carriage_dim_add_symmetry.x/2-carriage_dim_add.x
			...
			,-carriage_dim.z-carriage_dim_add_symmetry.z/2+carriage_filament_corr]
			,[0,0,0]];
...
module proto_mk8_stand_filamentsensor()
{
	filament_sensor(op=2);
}
			...
			union()
			{
				difference()
				{
					union()
					{
						...
						fftube_out=2.2;
						translate([0,0,carriage_filament_corr])
						...
						translate_rotate([[0,-carriage_dim.y/2-pc4_out,0],[-90,0,0]])
						{
							hull()
							{
								...
								cut=6;
								translate ([0,8.6+carriage_filament_corr+cut/2,0])
			...
			translate_rotate(carriage_tr) 
			{
				...
				for (y=[-1,1])
					...
					rotate ([0,180,90])
					{
						...
						translate ([0,0,screw-nut_offs+carriage_filament_corr])
						rotate ([0,0,90+90*x])
...
module mk8_stand_latch_fix(stand_latch_fix_nuts=true)
{
	for (y=[-1,1])
	translate([stand_sensor_tr[0].x,slot_tr[0].y+10*y,mk8_stand_tr[0].z])
	{
		latch_screw=12;
		m3_screw(h=latch_screw);
		
		if (stand_latch_fix_nuts)
			translate ([0,0,latch_screw-2])
			rotate ([0,0,-90])
				m3_square_nut();
	}
}
	...
	union()
	{
		difference()
		{
			union()
			{
				...
				translate_rotate (mk8_stand_tr)
				for (y=[0:1])
					translate ([mk8_stand_dim.x,10+20*y,0])
					rotate ([0,-90,0])
						lgroove(mk8_stand_dim.x,max_height=0.8);
				*/
			...
			mk8_stand_latch_fix(stand_latch_fix_nuts=stand_latch_fix_nuts);
...
module filament_sensor(op=-1)
{
	if (op==1)
	{
		translate_rotate(stand_sensor_tr)
		{
			rpi352(op=1);
	...
	{
		translate_rotate(stand_sensor_tr)
			opt_filament_path(filament_diameter=filament_melt_diameter);
	}
	else
	if (op==4)
	{
		...
		{
			translate_rotate(stand_sensor_tr)
				rp352_sensor_latch(filament_diameter=filament_diameter,filament_y_correction=0);
			mk8_stand_latch_fix();
		}
	}
}
module filament_sensor_latch()
{
	filament_sensor(op=4);
}
...
/*
module frontier_filament_sensor(op=-1)
{
	out=-4;
	frontier_filament_sensor_tr=[vector_replace(frontier_global_tr[0],1,frontier_axis_tr[0].y),[90,-90,0]];
	filament_sensor_corr=[-0.4,0,0];
	if (op==1)
	{
		translate(filament_sensor_corr)
		translate_rotate(frontier_filament_sensor_tr)
		{
			rpi352(op=1);
			translate ([0,out,0])
	...
	if (op==5)
	{
		...
		{
			translate ([-out,0,0])
			translate(frontier_filament_sensor_tr[0])
			for (y=[-1,1])
			translate ([0,9*y,0])
			rotate ([0,-90,0])
			{
...
module frontier_filament_sensor_latch()
{
	frontier_filament_sensor(op=4);
}
*/
...
module sensor_latch_spacer(height,out=[0,0],corr=[0,0])
{
	offs=0.6;
	zz=-7.2-0.5;
	dim1=[7.6-corr.x,height,10-corr.y];
	dim2=[dim1.x-offs*2+out[0],dim1.y+2,3.6+out[1]-corr.y];
	difference()
	{
		intersection()
		{
			translate ([0,10,0])
				rpi352(op=1);
			translate ([-dim1.x/2,-dim1.y,zz])
				cube (dim1);
		}
		translate ([-dim2.x/2-out[0]/2,-dim2.y+1,zz-offs-dim2.z+10-corr.y])
			cube (dim2);
	}
}
module stand_sensor_latch_spacer()
{
	sensor_latch_spacer(8.2,out=[10,0],corr=[0.2,0.2]);
}
/*
module frontier_sensor_latch_spacer()
{
	sensor_latch_spacer(10,out=[0,12]);
}
*/
module stand_sensor_latch_spacer_alt()
{
	cube ([4,8.2,2]);
}
...
if (part=="")
{
	...
	if (1)
	{
		...
		//proto_cutter_motor();
		proto_cutter();
		//proto_mk8_stand_filamentsensor();
...
else
{
	...
	if (part=="stand_bottom_channel")
		rotate ([0,0,0])
		mk8_stand_bottom_channel();
	if (part=="stand_channel_latch")
		rotate ([0,0,0])
		filament_sensor_latch();
	if (part=="stand_channel_latch_spacer")
		rotate ([0,90,0])
		stand_sensor_latch_spacer();
	if (part=="stand_channel_latch_spacer_alt")
		stand_sensor_latch_spacer_alt();
