...
module m3_square_nut_local(out=20,offs=0.2)
{
...
screws_disposition = 3; // [0:None, 1:1 screw, 2:2 screws at 90°, 3:3 screws at 120°, 4:4 screws at 90°]
...
//	translate ([0,0, base_height + toothed_part_length + retainer_height ]) rotate ([0,180,0])
	...
	difference()
	 {	 
		...
		if ( belt_retainer > 0 ) 
		{
			...
			rotate_extrude($fn=number_of_teeth*4)  
				polygon([
					[0,0]
					,[pulley_OD/2,0]
					,[pulley_OD/2 + retainer_height, retainer_height]
					,[pulley_OD/2 + retainer_height, retainer_height + retainer_upheight]
					,[0,retainer_height + retainer_upheight]
					,[0,0]
				]);
		}
		...
		translate([0,0,-1])
			cylinder(r=motor_shaft/2
					,h=base_height+toothed_part_length+retainer_height+retainer_upheight+2
					,$fn=motor_shaft*4);
		...
		if ( (base_diameter - motor_shaft)/2 < nut_depth + 3 ) { echo ("CAN'T DRAW CAPTIVE NUTS, DIAMETER TOO SMALL FOR NUT DEPTH!!!"); } else {
			...
			{
				//entrance
				translate([0,0,motor_shaft/2+nut_depth/2+nut_shaft_distance]) 
				//translate([0,-base_height/4-0.5,0]) 
				{
				...
				*/
				//grub screw hole
				hh=base_diameter/2;
				cylinder(r=screw_diameter/2,h=hh+1,$fn=80);
				translate ([-15,-15,hh-1])
					cube ([30,30,10]);
			}
		}}
	 }
