...
module ss443a_fd(op="cut")
{
	...
	fix_dim=[2,5];
	fix_flag=[fix_dim.x-0.3,2,6.6];
	fix_flag_cut=[1,1.6,1];
		...
		translate (g_tr)
		{
			...
			difference()
			{
				...
				union()
				{
					...
					translate ([-fix_flag.x/2,0,0])
					difference()
					{
						dim=[fix_flag.x,fix_flag.z,fix_flag.y];
						intersection()
						{
							//#cube (dim);
							translate ([0,0,dim.z])
						...
						translate ([-0.1,fix_flag.z-fix_flag_cut[1]-fix_flag_cut[0],-fix_flag.y+fix_flag_cut[2]])
							cube ([fix_flag.x+0.2,fix_flag_cut[1],fix_flag.y]);
					}
				}
	...
	if (op=="cut")
	{
		translate (g_tr)
		{
			...
			union()
			{
				hh=ptfe_height+ball/2;
				hull()
				{
					sphere (d=ball,$fn=60);
					cylinder (d=ptfe_diameter,h=hh,$fn=60);
				}
				yy=10;
				translate ([-fix_dim.x/2,-yy,hh-fix_dim.y])
					cube ([fix_dim.x,yy,fix_dim.y+0.01]);
			}
			
			fix_down=2;
			rotate([0,0,180])
			translate ([0,0,pol-fix_down-0.01])
			difference()
			{
				cdown=1;
				translate ([0,fix_flag.z-fix_flag_cut[1]-fix_flag_cut[0]-9,fix_flag_cut[2]-cdown])
				{
					dcut=fix_flag_cut[2]*2;
					dd=6;
					translate ([0,fix_flag_cut[1]+10,0])
					rotate ([90,0,0])
					difference()
					{
						hull()
						{
							cylinder (d=dd,h=2,$fn=60);
							translate([0,10,0])
								cylinder (d=dd+16,h=2,$fn=60);
						}
						
						translate ([0,0,-0.1])
						hull()
						{
							cylinder (d=dcut,h=10,$fn=60);
							translate([0,10,0])
								cylinder (d=dcut,h=10,$fn=60);
						}
					}
	...
	{		
		...
		union()
		{
			...
			/*
			rotate([0,0,180])
			translate ([-fix_flag.x/2,0,pol-0.01])
			difference()
			{
				dim=[fix_flag.x,fix_flag.z,fix_flag.y];
				intersection()
				...
				translate ([-0.1,fix_flag.z-fix_flag_cut[1]-fix_flag_cut[0]-10,-fix_flag.y+fix_flag_cut[2]])
					cube ([fix_flag.x+0.2,fix_flag_cut[1]+10,fix_flag.y]);
			}
			*/
		}
	}
